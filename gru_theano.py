...
class GRUTheano:
    ...
    def __init__(self, word_dim, hidden_dim=100, wordvec=None, bptt_truncate=-1):
    ...
    def __theano_build__(self):
        ...
        def forward_prop_step(x_t, s_t_prev, s_t2_prev):
            ...
            # GRU Layer 1
            z_t1 = T.nnet.sigmoid(U[0].dot(x_e) + W[0].dot(s_t_prev) + b[0])
            r_t1 = T.nnet.sigmoid(U[1].dot(x_e) + W[1].dot(s_t_prev) + b[1])
            c_t1 = T.tanh(U[2].dot(x_e) + W[2].dot(s_t_prev * r_t1) + b[2])
            s_t1 = (T.ones_like(z_t1) - z_t1) * s_t_prev + z_t1 * c_t1
